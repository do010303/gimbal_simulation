#!/usr/bin/env python3
"""
M4 tier-2 — full closed-loop monitor for SPLIT DOMAIN (PASSIVE).

Fork of docs/m3_box_handshake_test/m3_full_loop_monitor.py for the case where
box and drone run on two different ROS_DOMAIN_ID, bridged by DDS-Router (or
dib_split_bridge). Run this single process on the DRONE domain only.

Why not just run m3_full_loop_monitor.py as-is: it subscribes /box/state
directly, which is a box-local topic NOT part of the 3 bridged interfaces
(see docs/m4.md Phu luc A) -- a process on the drone domain never sees it.
BoxTelemetry (bridged, box->drone) already carries a full BoxState field
inside it (dib_msgs/msg/BoxTelemetry.msg), so this monitor reads box state
from msg.box_state.state on /b2/telemetry instead of subscribing /box/state
directly. Everything else (drone FSM, drone telemetry, MAVROS extended
state) is already local to the drone domain, same as M3.

Publishes NOTHING and calls NO service -- same passive-observer discipline
as M3's driver, so a PASS cannot be an artifact of the harness poking the
system.

Watches (drone domain only):
  /lander/state          drone FSM  (std_msgs/String)
  /b2/telemetry           box telemetry, BRIDGED -- box_state read from here
  /d1/telemetry           drone telemetry produced by the M2 bridge, BRIDGED
  /mavros/extended_state  ground truth for landed_state (drone-local)

Pass criteria: identical to m3_full_loop_monitor.py's 8 criteria.
...
from dib_msgs.msg import BoxTelemetry, DroneTelemetry
...
class Monitor(Node):
    def __init__(self):
        super().__init__('m4_full_loop_monitor')
        self.t0 = time.time()
    ...
    def on_box_tlm(self, msg):
        # box_state arrives inside the bridged BoxTelemetry message itself --
        # this is the split-domain substitute for subscribing /box/state.
        state = msg.box_state.state
        if not self.box_trace or self.box_trace[-1][1] != state:
            name = BOX_NAMES.get(state, f'UNKNOWN({state})')
            self.box_trace.append((self.el(), state))
            print(f'[{self.el():7.1f}s] BOX    -> {name}({state})  (via bridged b{BOX_ID}/telemetry)')
...
def main():
    rclpy.init()
    node = Monitor()
    print('\n=== M4 tier-2 full closed-loop monitor (passive, split-domain, run on DRONE side) ===')
    ...
    print(' -> '.join(f'{BOX_NAMES.get(s, s)}({s})' for _, s in bt) if bt
          else '(nothing on b{}/telemetry.box_state)'.format(BOX_ID))
    ...
    results.append(('Box reached CHARGING (drone requested power off)',
                    ok7,
    ...
    results.append(('Box GPS was valid (non-zero) — /gps fixture running, seen via bridge',
                    node.box_gps_seen,
    ...
    print(f'\n=== M4 tier-2 full closed loop (split-domain): {"PASS" if overall else "FAIL"} ===\n')
